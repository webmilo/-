	...
	a.showShareTip = function() {
		if (!h.noshare) {
			if (null == f) {
                ...
                if(navigator.userAgent.match(/fapiaotong/gi) != null){
                    c = new createjs.Bitmap(a.queue.getResult("SharefapiaotongTip"));
                    c.x = W - c.getBounds().width;
                    c.y = H - c.getBounds().height;
                }
				f.addChild(c);
...
})(window.fapiaotongStage = window.fapiaotongStage || {});
...
function MyProgressBar(a, d, b) {
	...
	b.textAlign = "center";
	this.addChild(b);
	b = new createjs.Text("fapiao.com", "32px Arial", "black");
	b.x = a / 2;
	b.y = d - 100;
	b.textAlign = "center";
	this.addChild(b)
